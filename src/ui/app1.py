...
def main():
 ...
 # Auto-generate content based on RL recommendation (if available) or preference
 if survey_completed:
 # Try to get RL recommendation first
 rec_result = st.session_state.manager.process_user_request(
 "recommend",
 {},
 st.session_state.session_id
 ...
 if rec_result.get("success"):
 recommended_mode = rec_result.get("recommended_mode", preference)
 # Use recommendation if it's different from preference, or if preference is unknown
 if recommended_mode and (preference == LearningMode.UNKNOWN.value or recommended_mode != preference):
 generate_content_for_mode(recommended_mode)
 elif preference != LearningMode.UNKNOWN.value:
